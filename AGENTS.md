# AGENTS.md — working in GR-ai-agents
...
agents/<name>.md                ← Codex subagents
hooks/                          ← hook scripts + the settings.json snippets that arm them
references/*.md                 ← shared material, installed to ~/.Codex/GR-references/
...
`~/.Codex/skills/GR-tdd/SKILL.md` is editing this repo's working tree — they'll see it in
...
- **Codex runs on the host; the toolchain does not.** Every build/test/lint command is
...
- **Two artifact trees.** Committed standing knowledge in `<repo>/docs/{agents,adr}` plus
  `CONTEXT.md`; per-feature work in `<scratch>/<feature>/{grill,spec,plans}/`. Reach decides the
...
- **Agent docs have named consumers.** A skill names the file it needs — `repo-gotchas.md`,
  `testing.md`, `lint-and-precommit.md` — and the repo's `AGENTS.md` carries the table pointing at
