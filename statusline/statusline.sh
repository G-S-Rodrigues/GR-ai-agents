#!/bin/bash
# Vendored from: https://github.com/daniel3303/ClaudeCodeStatusLine (v1.4.4)
# Local changes — search "LOCAL CHANGE" below for each edit:
#   - segment order is CWD@Branch, Model, Tokens, Effort, 5h, 7d (upstream puts Model first)
#   - CWD@Branch is "dir 🌿 branch" instead of "dir@branch"
#   - file-change count (+/-) moved to the very last segment instead of inline after the branch
#   - Extra and the trailing CLI-version segment are dropped entirely
#   - a session-duration timer is inserted between Effort and 5h (from cost.total_duration_ms)
#
# Single line: cwd 🌿 branch | model | tokens (%used) | effort | session | 5h @reset | 7d @reset | +/-
...
# Format a duration in milliseconds as e.g. "1h 23m", "23m", or "45s"
format_duration() {
    local ms=$1
...
# LOCAL CHANGE: upstream renders Model first, then "| CWD@Branch". We render
# CWD@Branch first (no leading separator) and Model second, so the separator
# moves to trail the CWD@Branch block instead of leading it.
# LOCAL CHANGE: branch is joined with a 🌿 instead of "@", and the file-change
# count (+/-) is held in $git_stat and rendered as the very last segment,
# instead of inline right after the branch name.
out=""
git_stat=""
...
# LOCAL CHANGE: session duration timer, sourced from cost.total_duration_ms
# (wall-clock time since session start, provided directly in the stdin JSON).
session_duration_ms=$(echo "$input" | jq -r '.cost.total_duration_ms // empty')
if [ -n "$session_duration_ms" ] && [ "$session_duration_ms" != "null" ]; then
    session_duration=$(format_duration "$session_duration_ms")
    out+=" ${dim}|${reset} "
    out+="${white}session${reset} ${cyan}${session_duration}${reset}"
fi
...
# LOCAL CHANGE: file-change count (+/-), captured earlier alongside CWD@Branch,
# renders as the very last segment instead of inline right after the branch name.
[ -n "$git_stat" ] && out+="${sep}${green}${git_stat%% *}${reset} ${red}${git_stat##* }${reset}"
